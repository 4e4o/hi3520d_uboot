...
WORK_DIR="/home/computer/workspace/my_openipc"
...
mkdir -p "${BUILD_DIR}"
cd "${BUILD_DIR}"
PATH=$PATH:${TOOLCHAIN} make O="${BUILD_DIR}" -C "${LOADER_SRC_DIR}" ARCH=arm CROSS_COMPILE=${CROSSRULE} distclean
PATH=$PATH:${TOOLCHAIN} make O="${BUILD_DIR}" -C "${LOADER_SRC_DIR}" ARCH=arm CROSS_COMPILE=${CROSSRULE} hi3520d_config
PATH=$PATH:${TOOLCHAIN} make O="${BUILD_DIR}" -C "${LOADER_SRC_DIR}" ARCH=arm CROSS_COMPILE=${CROSSRULE} PLATFORM_REFLAGS="-std=gnu90" STAGING_DIR={STAGINDIR} -j$((`nproc`+1))   #>.build_uboot.log 2>&1
